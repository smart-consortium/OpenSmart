...
<nav class="large-3 medium-4 columns" id="actions-sidebar">
    <ul class="side-nav">
        ...
        <li><?= $this->Html->link(__('Upload Video'), ['action' => 'upload']) ?></li>
...
<div class="videos index large-9 medium-8 columns content">
    ...
    <table cellpadding="0" cellspacing="0">
        ...
        <tbody>
            ...
            <tr>
                <td rowspan="2"><?= $this->Number->format($video->id) ?></td>
                <td><?= $video->has('user') ? $this->Html->link($video->user->name, ['controller' => 'Users', 'action' => 'view', $video->user->id]) : '' ?></td>
                <td><?= $video->has('server') ? $this->Html->link($video->server->name, ['controller' => 'Servers', 'action' => 'view', $video->server->id]) : '' ?></td>
                <td><?= VideoViewHelper::mode($video->mode) ?></td>
                <td><?= VideoViewHelper::status($video->status) ?></td>
                ...
                <td class="actions">
                    ...
                    <?= $this->Html->link(__('Rebuild'), ['action' => '', $video->id],['confirm' => __('Not implement')]) ?>
                </td>
            </tr>
            <tr>
                <td colspan="7" style="text-align:left">
	                <?= $this->Html->link($video->svid, ['action' => 'view', $video->id]) ?>
                </td>
            </tr>
            <?php endforeach; ?>
        </tbody>
